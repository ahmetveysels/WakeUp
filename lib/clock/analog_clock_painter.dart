...
class AnalogClockPainter extends CustomPainter {
  ...
  AnalogClockPainter({required this.datetime, this.showDigitalClock = true, this.showTicks = true, this.showNumbers = true, this.showSecondHand = true, this.hourHandColor = Colors.black, this.minuteHandColor = Colors.black, this.secondHandColor = Colors.redAccent, this.tickColor = Colors.grey, this.digitalClockColor = Colors.black, this.numberColor = Colors.black, this.showAllNumbers = false, this.textScaleFactor = 1.0, this.useMilitaryTime = true});
  ...
  void _paintClockHands(Canvas canvas, Size size, double scaleFactor) {
    ...
    if (showSecondHand) {
      canvas.drawLine(size.center(_getHandOffset(seconds, HAND_PIN_HOLE_SIZE * scaleFactor)), size.center(_getHandOffset(seconds, longHandLength)), handPaint..color = secondHandColor);
    }
  }
  ...
  void _paintDigitalClock(Canvas canvas, Size size, double scaleFactor, bool useMilitaryTime) {
    ...
    TextSpan digitalClockSpan = TextSpan(style: TextStyle(color: digitalClockColor, fontFamily: "ClockFont", fontSize: 33 * scaleFactor * textScaleFactor), text: "$hour:$minute:$second$meridiem");
